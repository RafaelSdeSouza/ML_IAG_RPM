################ ANALISE 8 (12-ABR-2018)
#rm(list=ls())

# setwd('/home/allan/Documents/astro/MLG_IAG_USP/analise8')

# -------------------------- SEM ELIMINAR OUTLIERS dos datasets 1 a 3 e eliminando do 4 ---------------------
...
# !! atualizacao de abr/18: inverter p1 com p2 e tirar o título
x11()
...
savePlot("fig1_GMM_2a2_combinada.PNG", type = "png",
         device = dev.cur())
...
grid.arrange(pe2[[1]],pe2[[2]],pe2[[3]],pe2[[4]],pe2[[5]],pe2[[6]],
             pe1[[1]],pe1[[2]],pe1[[3]],pe1[[4]],pe1[[5]],pe1[[6]],
             pe3[[1]],pe3[[2]],pe3[[3]],pe3[[4]],pe3[[5]],pe3[[6]],
             pe4[[1]],pe4[[2]],pe4[[3]],pe4[[4]],pe4[[5]],pe4[[6]], ncol=6)

savePlot("fig2_GMM_2a2_elipses_combinada.PNG", type = "png",
         device = dev.cur())
...
p4_icl_new <-
  ...
  ggtitle("Dataset 4")+
  theme(plot.title = element_text(hjust=0.5))+
  annotate("text", x=c(1.7, 2.7, 3.7, 4.7), y=c(7600, 5000, 4000, 4000),
           label=var_icl_df4) # pcts estao no automatico
...
grid.arrange(p2_icl_new, p1_icl_new, p3_icl_new, p4_icl_new, ncol=2,
             top = "Increase/Decrease in ICL when removing/leaving Variables in the GMM Model")

savePlot("fig3_icl_combinada.PNG", type = "png",
         device = dev.cur())
...
p4_p <- ggplot(data4.df, aes(Bmag, Sint))+
  ...
  scale_fill_gradientn (colors = c('#E69F00', '#56B4E9', "#4F687F"),
                        labels=c('','group 2',0.5,'group 1', ''))+
  # ggtitle("Dataset 4 - Probability of belonging to group 1 and 2")+
  ggtitle("Dataset 4")+
  theme(plot.title = element_text(hjust=0.5))+
  labs(fill='probability') # para alterar título da legenda
...
# !! atualizacao de abr/18: inverter p1 com p2, tirar o título e fazer em uma unica coluna
x11()
grid.arrange(p2_p,p1_p,p3_p,p4_p, ncol=1)

savePlot("fig4_prob_clusters_combinada_v2.PNG", type = "png",
         device = dev.cur())
...
# !! atualizacao de abr/18: inverter p1 com p2, tirar o título e fazer em uma unica coluna
grid.arrange(p2_t,p1_t,ncol=2)


savePlot("fig5_gruposAGNS_com_outliers(combinada).PNG", type = "png",
         device = dev.cur())

###### fim da analise com outliers

# !! atualizacao de abr/18: fazer figuras 1 e 2 para GMM G=3 nas dimensões da figura de probabilidades


## GMM com G=3 para todos
library(mclust)
# dataset1
set.seed(1984)
out1_3g <- Mclust(data1.df[,1:4], G=3)
data1.df$cluster_3g <- as.factor(out1_3g$classification)
# dataset2
set.seed(1984)
out2_3g <- Mclust(data2.df[,1:4], G=3)
data2.df$cluster_3g <- as.factor(out2_3g$classification)
# dataset3
set.seed(1984)
out3_3g <- Mclust(data3.df[,1:4], G=3)
data3.df$cluster_3g <- as.factor(out3_3g$classification)
# dataset4
set.seed(1984)
out4_3g <- Mclust(data4.df[,1:4], G=3)
data4.df$cluster_3g <- as.factor(out4_3g$classification)
...
# GMM 2x2 para 3G
# dataset1
p1_3g <- ggplot(data1.df, aes(Core, B_Band))+
  geom_point(aes(colour = cluster_3g, shape=R1), size = 1.2)+
  scale_shape_manual(values=c(1, 3))+
  scale_color_manual(values=c('#E69F00', '#56B4E9', 'purple'))+
  ggtitle("Dataset1")+
  theme(plot.title = element_text(hjust=0.5))

# dataset2
p2_3g <- ggplot(data2.df, aes(Radio, B_Band))+
  geom_point(aes(colour = cluster_3g, shape=R1), size = 1.2)+
  scale_shape_manual(values=c(1, 3))+
  scale_color_manual(values=c('#E69F00', '#56B4E9', 'purple'))+
  ggtitle("Dataset2")+
...
# dataset3
p3_3g <- ggplot(data3.df, aes(VLA_6cm, I_Band))+
  geom_point(aes(colour = cluster_3g, shape=R1), size = 1.2)+
  scale_shape_manual(values=c(1, 3))+
  scale_color_manual(values=c('#E69F00', '#56B4E9', 'purple'))+
  ggtitle("Dataset3")+
...
# dataset4
p4_3g <- ggplot(data4.df, aes(Sint, Bmag))+
  geom_point(aes(colour = cluster_3g), size = 1.2, shape=1)+
  #scale_shape_manual(values=c(1, 3))+
  scale_color_manual(values=c('#E69F00', '#56B4E9', 'purple'))+
  ggtitle("Dataset4")+
  theme(plot.title = element_text(hjust=0.5))
...
library(gridExtra)
x11()
grid.arrange(p2_3g,p1_3g,p3_3g,p4_3g, ncol=1)

savePlot("fig6_GMM_3G_combinada.PNG", type = "png",
         device = dev.cur())


## GMM 2x2 para 3G com elipses:

# dataset1
pe1_3g <- ggplot(data1.df, aes(Core, B_Band))+
  geom_point(aes(colour = cluster_3g, shape=R1), size = 1.2)+
  scale_shape_manual(values=c(1, 3))+
  scale_color_manual(values=c('#E69F00', '#56B4E9', 'purple'))+
  stat_ellipse(aes(Core, B_Band, color=cluster_3g), type="norm" )+
  ggtitle("Dataset1")+
  theme(plot.title = element_text(hjust=0.5))

# dataset2
pe2_3g <- ggplot(data2.df, aes(Radio, B_Band))+
  geom_point(aes(colour = cluster_3g, shape=R1), size = 1.2)+
  scale_shape_manual(values=c(1, 3))+
  scale_color_manual(values=c('#E69F00', '#56B4E9', 'purple'))+
  stat_ellipse(aes(Radio, B_Band, color=cluster_3g), type="norm" )+
  ggtitle("Dataset2")+
  theme(plot.title = element_text(hjust=0.5))

# dataset3
pe3_3g <- ggplot(data3.df, aes(VLA_6cm, I_Band))+
  geom_point(aes(colour = cluster_3g, shape=R1), size = 1.2)+
  scale_shape_manual(values=c(1, 3))+
  scale_color_manual(values=c('#E69F00', '#56B4E9', 'purple'))+
  stat_ellipse(aes(VLA_6cm, I_Band, color=cluster_3g), type="norm" )+
  ggtitle("Dataset3")+
  theme(plot.title = element_text(hjust=0.5))

# dataset4
pe4_3g <- ggplot(data4.df, aes(Sint, Bmag))+
  geom_point(aes(colour = cluster_3g), size = 1.2, shape=1)+
  #scale_shape_manual(values=c(1, 3))+
  scale_color_manual(values=c('#E69F00', '#56B4E9', 'purple'))+
  stat_ellipse(aes(Sint, Bmag, color=cluster_3g), type="norm" )+
  ggtitle("Dataset4")+
  theme(plot.title = element_text(hjust=0.5))

library(gridExtra)
x11()
grid.arrange(pe2_3g,pe1_3g,pe3_3g,pe4_3g, ncol=1)

savePlot("fig7_GMM_3G_elipses_combinada.PNG", type = "png",
         device = dev.cur())
